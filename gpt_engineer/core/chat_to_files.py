...
def _strip_agent_noise(chat: str) -> str:
    """
    Drop CrewAI / chain-of-thought preamble so the parser sees file blocks first.
    Keeps from the first 'File:' onward (canonical output format).
    """
    if "File:" not in chat:
        return chat
    idx = chat.find("File:")
...
def _normalize_logged_chat(chat: str) -> str:
    """
    Normalize streamed Crew-style logs into plain lines before parsing.
    Example: "💻 Developer │ File: src/main.py" -> "File: src/main.py"
    """
    normalized_lines = []
...
def chat_to_files_dict(chat: str) -> FilesDict:
    ...
    normalized_chat = _normalize_logged_chat(_strip_agent_noise(chat))

    # Preferred format used by Crew tasks:
    # File: <filename>
    # ```lang
    # <code>
    # ```
    ...
    for match in file_block_pattern.finditer(normalized_chat):
        path = _clean_path(match.group(1))
        content = match.group(2).strip()
        if path and content:
            files_dict[path] = content

    # Backwards-compatible fallback for plain filename + code fence format.
    if not files_dict:
        legacy_pattern = re.compile(
            r"(\S+)\r?\n\s*```[^\r\n]*\r?\n(.+?)```", re.DOTALL
        )
        for match in legacy_pattern.finditer(normalized_chat):
            path = _clean_path(match.group(1))
            content = match.group(2).strip()
            if path and content:
                files_dict[path] = content
